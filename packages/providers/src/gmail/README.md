...
`GmailSendAdapter` supports `sendEmail` and is only intended for the third dedicated Nexi mailbox. Server code calls it from the ApprovalQueue executor after a pending email artifact is approved.
...
- `GMAIL_SEND_MAILBOX_EMAIL`, `_ALIAS`, `_REFRESH_TOKEN`: dedicated Nexi sender mailbox, consented with `gmail.send`.
