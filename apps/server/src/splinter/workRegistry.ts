...
export class SplinterWorkSelector {
  ...
  async reconcile(id: string, evidenceRefs: string[]) { const item = await this.work.get(id); if (!item || item.status !== "CLAIMED" || !item.activeSplinterJobId || evidenceRefs.length === 0) throw new Error("Work completion evidence is insufficient."); const job = await this.jobs.get(item.activeSplinterJobId); if (!job || job.state !== "SUCCEEDED" || job.result !== "PASS") throw new Error("Linked Splinter job is not complete."); const adjudication = splinterLifecycleController.authorizeWorkCompletion(job, evidenceRefs); if (!adjudication.allowed) throw new Error(adjudication.requiredAction ?? adjudication.reason); return this.work.update(id, { status: "COMPLETED", completedEvidenceRefs: evidenceRefs, activeSplinterJobId: undefined }); }
