...
import { idSchema, splinterDeploymentStatusSchema, splinterExecutionModeSchema, splinterIntegrationStatusSchema, splinterReconciliationEvidenceSchema, splinterRepairProofInjectionSchema, splinterRepositoryTargetSchema, splinterRequiredCheckSchema, splinterReviewSchema, splinterRfiSchema, splinterWorkerResultSchema } from "@nexteam/core";
import { z } from "zod";
...
export function registerSplinterRelayRoutes(app: Express, deps: SplinterRelayRouteDeps): void {
  ...
  app.post("/api/internal/splinter/programs/:id/heartbeat", async (req, res) => {
    if (!deps.programService || !authorized(req, env)) return reject(res, 401, "Splinter relay authorization is required.");
    const parsed = z.object({ workerId: z.string().min(1).max(128), leaseMs: z.number().int().min(1000).max(900000).optional() }).strict().safeParse(req.body);
