...
export class SplinterProgramService {
  ...
  async reconcile(id: string, currentStagingSha: string): Promise<ProgramReconcileResult> {
    const program = await this.require(id); if (program.state !== "ACTIVE") return { program };
    ...
    const state: SplinterProgramState = unfinished.length === 0 ? "COMPLETE" : actions.length > 0 ? "GLOBAL_OWNER_REQUIRED" : "GLOBAL_EXTERNAL_BLOCKER";
    return { program: await this.persistRequired(program, { state, ownerActionQueue: actions, terminalReason: state === "COMPLETE" ? "All program work items are complete." : "All remaining program work is scoped-blocked.", nextAction: state === "COMPLETE" ? "Program complete." : "Await the scoped blocker resolution." }, "PROGRAM_RECONCILED") };
  }
  async claimWorker(id: string, workerId: string, leaseMs = 300000) {
    ...
    const now = this.now(); return this.persistRequired(program, { workerLease: { workerId, workItemId: program.activeWorkItemId, claimedAt: now, heartbeatAt: now, expiresAt: new Date(Date.parse(now) + leaseMs).toISOString() } }, "WORKER_CLAIMED");
  }
