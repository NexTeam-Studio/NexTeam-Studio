...
export class SplinterProgramService {
  ...
  async consumeApproval(id: string, approvalId: string, scopeFingerprint: string) { const consumed = await this.programs.consumeApproval(id, approvalId, scopeFingerprint, this.now()); if (!consumed) throw new Error("Approval is unavailable or already consumed."); const item = await this.approvalWork(consumed, scopeFingerprint); const released = await this.work.update(item.workItemId, { status: "APPROVED", ownerDecisionRequired: false, blockedBy: undefined }); if (!released) throw new Error("Approved work item disappeared."); return this.persistRequired(consumed, { ownerActionQueue: consumed.ownerActionQueue.map(action => action.workItemId === item.workItemId ? { ...action, state: "RESOLVED", resolvedAt: this.now() } : action), nextAction: `Reconcile approved work ${item.workItemId}.` }, "APPROVAL_CONSUMED"); }
