...
import { FirestoreSplinterProgramRepository, SplinterProgramService } from "./splinter/programService.js";
import { FirestoreWorkRegistry, SplinterWorkSelector } from "./splinter/workRegistry.js";
...
const splinterWorkRegistry = adminDb ? new FirestoreWorkRegistry(adminDb) : null;
const splinterWorkSelector = splinterWorkRegistry && splinterRepository ? new SplinterWorkSelector(splinterWorkRegistry, splinterRepository) : null;
const splinterProgramService = adminDb && splinterRepository && splinterWorkRegistry && splinterWorkSelector
  ? new SplinterProgramService(new FirestoreSplinterProgramRepository(adminDb), splinterWorkRegistry, splinterRepository, splinterWorkSelector)
  : null;
...
if (splinterRepository && splinterService && splinterWorkRegistry && splinterWorkSelector) registerSplinterRelayRoutes(app, { repository: splinterRepository, service: splinterService, workRegistry: splinterWorkRegistry, workSelector: splinterWorkSelector, ...(splinterProgramService ? { programService: splinterProgramService } : {}), env: process.env });
