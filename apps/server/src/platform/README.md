...
This folder owns NexTeam's platform layer: plan catalog, tool entitlements, tenant billing status, tenant admin data, multi-user access records, per-tenant cost summaries, and Firestore backup/export routes.
...
- `routes.ts` exposes the platform/operator API and tenant owner/admin multi-user endpoints. User and job-link routes must resolve `AccessContext` before writing.
