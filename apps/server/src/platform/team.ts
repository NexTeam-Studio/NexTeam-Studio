...
export const platformUserSchema = z.object({
  ...
  telephone: z.string().trim().min(1).max(50).optional(), address: z.object({ line1: z.string().trim().min(1).max(200), line2: z.string().trim().max(200).optional(), city: z.string().trim().min(1).max(100), region: z.string().trim().min(1).max(100), postalCode: z.string().trim().min(1).max(30), country: z.string().trim().min(2).max(2) }).optional(), profilePhotoRef: z.string().trim().min(1).max(500).optional(), twoFactorState: z.enum(["ENROLLED", "NOT_ENROLLED"]).default("NOT_ENROLLED"),
