...
export const platformUserSchema = z.object({
  ...
  role: platformRoleSchema, capabilityOverrides: platformCapabilityOverrideSchema.default({ grant: [], deny: [] }), accountStatus: z.enum(["ACTIVE", "DISABLED"]), createdAt: z.string().datetime(), updatedAt: z.string().datetime(), createdBy: z.string().min(1), updatedBy: z.string().min(1)
}).strict();
