...
export class FirestoreTimePayRepository implements TimePayRepository {
  ...
  async listEvents(tenantId: string, factId?: string) { const rows = await this.db.collection("timePayEvents").where("tenantId", "==", tenantId).get(); return rows.docs.map((d) => d.data() as TimePayEvent).filter((x) => !factId || x.factId === factId).sort((a, b) => a.occurredAt.localeCompare(b.occurredAt)); }
}
