...
<body id="top">
  ...
  <main>
    ...
    <section class="section" id="service-area"><div class="shell"><div class="section-heading"><p class="eyebrow">Service Area</p><h2>A Cleaner Property Starts With One Good Call</h2><p>Owens Bluewater Wash serves homeowners, businesses, property managers, and commercial properties throughout its local service region.</p></div><div class="area-grid"><article class="audience-card"><h3>Residential Properties</h3><p>Exterior cleaning that helps your home look well cared for.</p></article><article class="audience-card"><h3>Commercial Properties</h3><p>Professional cleaning for businesses, storefronts, and exterior surfaces.</p></article><article class="audience-card"><h3>Property Managers</h3><p>Clear, dependable service for the properties you oversee.</p></article></div><div class="tag-list">${badges.map((badge) => `<span>${escapeHtml(badge)}</span>`).join("")}</div></div></section>
    <section class="contact" id="contact"><div class="shell section contact-grid"><div class="contact-copy"><p class="eyebrow">Contact</p><h2>${escapeHtml(contact?.heading ?? "Ready for a Cleaner Property?")}</h2><p>${escapeHtml(contact?.intro ?? `Call ${phone} to talk through your project and request an estimate.`)}</p></div><div class="contact-actions"><a class="button" href="${escapeHtml(phoneHref)}">Call ${escapeHtml(phone)}</a>${website ? `<a class="button light" href="${escapeHtml(website)}" target="_blank" rel="noopener noreferrer">Visit ${escapeHtml(website.replace(/^https?:\/\//, "").replace(/\/$/, ""))}</a>` : ""}</div></div></section>
  </main>
  <footer class="footer"><div class="shell"><div class="footer-grid"><div><a class="footer-logo" href="#top" aria-label="Owens Bluewater Wash home">${logoMarkup}</a><p><strong>${escapeHtml(site.title)}</strong><br />Cleaner surfaces. Better impressions.</p><p><a href="${escapeHtml(phoneHref)}">${escapeHtml(phone)}</a><br />${website ? `<a href="${escapeHtml(website)}" target="_blank" rel="noopener noreferrer">${escapeHtml(website.replace(/^https?:\/\//, "").replace(/\/$/, ""))}</a>` : ""}</p></div><nav aria-label="Footer navigation"><a href="#services">Services</a><a href="#packages">Packages</a><a href="#service-area">Service Area</a><a href="#contact">Contact</a></nav></div><p class="copyright">© ${new Date().getFullYear()} ${escapeHtml(site.title)}. All rights reserved.</p></div></footer>
