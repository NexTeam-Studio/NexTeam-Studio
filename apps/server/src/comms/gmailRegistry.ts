...
function configFromEnv(env: NodeJS.ProcessEnv, prefix: string, fallbackAlias: string): GmailMailboxConfig | null {
  ...
  return {
    ...
    clientSecret,
    refreshToken,
    tenantId: value(env, "TENANT_ID") || "aquatrace"
  };
}
...
export function createCommsRailFromEnv(env: NodeJS.ProcessEnv): CommsRail {
  ...
  for (const [prefix, fallbackAlias] of [
    ...
    const config = configFromEnv(env, prefix, fallbackAlias);
    if (config) {
