...
test("NexCommand Team persists platform-only profiles, redacts sensitive data, and records immutable audit events", async () => {
  ...
  try {
    ...
    const created = await fetch(`${base}/api/platform/admin/team`, { method: "POST", headers, body: JSON.stringify({ authUid: "operator-two", firstName: "Avery", lastName: "Stone", email: "avery@example.test", telephone: "555-0100", address: { line1: "1 Main", city: "Town", region: "NY", postalCode: "10001", country: "US" }, role: "Administrator" }) });
...
test("NexCommand rejects a duplicate Firebase UID even when the existing internal profile is disabled", async () => {
  const repository = new InMemoryPlatformRepository();
  ...
  await assert.rejects(() => repository.savePlatformUser({
    ...
    createdBy: "system",
    updatedBy: "system"
  }), /only one platform or tenant profile/i);
  const resolved = await repository.getPlatformUserByAuthUid(authUid);
  assert.equal(resolved?.id, "platform_user_legacy_disabled");
  assert.equal(resolved?.accountStatus, "DISABLED");
});
...
test("protected Owner identity is immutable while profile maintenance and authoritative security fields persist", async () => {
  ...
  registerPlatformRoutes(app, { repository, storage, env: { NEXI_FIREBASE_AUTH_REQUIRED: "true" }, platformOperatorAuth: { async verifyIdToken() { return { uid: "owner", platform_operator: true }; } } });
  const server = app.listen(0);
  try {
    const base = `http://127.0.0.1:${server.address().port}`;
    const headers = { authorization: "Bearer owner", "content-type": "application/json" };
    const created = await fetch(`${base}/api/platform/admin/team`, { method: "POST", headers, body: JSON.stringify({ authUid: "owner", firstName: "Protected", lastName: "Owner", email: "Owner@NexTeam.Test", role: "Owner" }) });
    assert.equal(created.status, 201);
    const profilePhoto = await fetch(`${base}/api/platform/admin/team/me/profile-photo`, { method: "POST", headers: { authorization: "Bearer owner", "content-type": "image/png" }, body: Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]) });
    assert.equal(profilePhoto.status, 201);
    ...
    assert.equal((await fetch(`${base}/api/platform/admin/team`, { method: "POST", headers, body: JSON.stringify({ authUid: "another-user", firstName: "Another", lastName: "User", email: "OWNER@nexteam.test", role: "Administrator" }) })).status, 409);
    ...
    assert.equal(owner.user.telephone, "555-0200");
  } finally { await new Promise((resolve) => server.close(resolve)); }
});
