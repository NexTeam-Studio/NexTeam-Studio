  ...
  try {
    for (const [sourceTenantId, targetTenantId] of [[tenantA, tenantB], [tenantB, tenantA]]) {
      ...
      const routes = [
        ...
        `/api/fielddocs/bundles?tenantId=${targetTenantId}`,
        `/api/fielddocs/text-snippets?tenantId=${targetTenantId}`,
        `/api/fielddocs/signed-documents?tenantId=${targetTenantId}`,
