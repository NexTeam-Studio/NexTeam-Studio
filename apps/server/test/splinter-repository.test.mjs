...
test("single-use program approvals are consumed once and stale workers lose their lease", async () => {
  const work = new InMemoryWorkRegistry(); const jobs = new InMemorySplinterRepository(); const selector = new SplinterWorkSelector(work, jobs); const programs = new SplinterProgramService(new InMemorySplinterProgramRepository(), work, jobs, selector);
  await work.create({ workItemId: "approval-work", title: "Approval", goal: "Approval", module: "platform", tenantScope: "platform", priority: 1, launchCritical: false, dependencies: [], acceptanceCriteria: ["ok"], requiredChecks: ["SPLINTER_FOCUSED_TESTS"], allowedPaths: [], pathDiscoveryPolicy: "EXPLICIT_PATHS", executionMode: "READ_ONLY", repositoryTarget: "NEXTEAM", reviewRequired: false, maxAttempts: 1, ownerDecisionRequired: false, promotionPolicy: "NONE", sourceRequirementRefs: ["docs/a.md"], requirementRevision: "r1", nonPromotable: false, reconciliationMode: false, completedEvidenceRefs: [] }); await selector.approve("approval-work");
  const program = await programs.create({ objective: "Approval", workItemIds: ["approval-work"] }); await programs.grantApproval(program.programId, "single", "scope");
  const consumed = await Promise.allSettled([programs.consumeApproval(program.programId, "single", "scope"), programs.consumeApproval(program.programId, "single", "scope")]); assert.equal(consumed.filter(value => value.status === "fulfilled").length, 1);
});
