...
export function RemainingSettingsSections({
  ...
  return <section className="nexops-two-column" aria-label="Remaining settings sections">
    ...
    <article className="nexops-module-card">
      ...
      {value.customFields.map((field, index) => <div className="nexops-quote-toggle-grid" key={field.id}><label className="nexops-field"><span>Label</span><input value={field.label} onChange={(event) => update("customFields", value.customFields.map((entry, i) => i === index ? { ...entry, label: event.target.value } : entry))} /></label><label className="nexops-field"><span>Applies To</span><input value={field.appliesTo} onChange={(event) => update("customFields", value.customFields.map((entry, i) => i === index ? { ...entry, appliesTo: event.target.value } : entry))} /></label><label className="nexops-field"><span>Value Type</span><select value={field.valueType} onChange={(event) => update("customFields", value.customFields.map((entry, i) => i === index ? { ...entry, valueType: event.target.value as typeof field.valueType } : entry))}>{[["text", "Text"], ["true_false", "True / False"], ["area", "Area"], ["numeric", "Numeric"], ["dropdown_link", "Dropdown / Link"]].map(([id, label]) => <option value={id} key={id}>{label}</option>)}</select></label><Toggle checked={field.transferable} onChange={(transferable) => update("customFields", value.customFields.map((entry, i) => i === index ? { ...entry, transferable } : entry))}>Transfer across lifecycle</Toggle></div>)}
    </article>
