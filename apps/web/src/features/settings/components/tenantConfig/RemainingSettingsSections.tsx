...
export function RemainingSettingsSections({
  ...
  return <section className="nexops-two-column" aria-label="Remaining settings sections">
    <article className="nexops-module-card" hidden={activeSection !== "company"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "checklists-reports"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "automations"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "requests-booking"}>
      ...
      <div className="nexops-quote-toggle-grid"><label className="nexops-field"><span>Booking Buffer (minutes)</span><input type="number" min="0" value={value.requestsBooking.bufferMinutes} onChange={(event) => update("requestsBooking", { ...value.requestsBooking, bufferMinutes: Math.max(0, Number(event.target.value) || 0) })} /></label><label className="nexops-field"><span>Service Areas (comma-separated)</span><input value={value.requestsBooking.serviceAreas.join(", ")} onChange={(event) => update("requestsBooking", { ...value.requestsBooking, serviceAreas: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} /></label><label className="nexops-field"><span>New-request notification recipient</span><input type="email" placeholder="office@example.com" value={value.requestsBooking.internalNotificationRecipient ?? ""} onChange={(event) => update("requestsBooking", { ...value.requestsBooking, internalNotificationRecipient: event.target.value.trim() || undefined })} /></label></div>
      ...
      <Toggle checked={value.requestsBooking.publicSlotPickerEnabled} onChange={(publicSlotPickerEnabled) => update("requestsBooking", { ...value.requestsBooking, publicSlotPickerEnabled })}>Enable public date/time slot picker</Toggle>
      <p className="nexops-form-note">The slot picker is not yet available. Keep this off until the booking-slot experience is built.</p>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "tax"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "custom-fields"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "schedule"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "nexportal"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "payments"}>
    ...
    <article className="nexops-module-card" hidden={activeSection !== "integrations"}>
