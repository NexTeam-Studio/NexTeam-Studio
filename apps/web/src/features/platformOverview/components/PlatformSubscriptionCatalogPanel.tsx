...
const nexTeamModules = ["NexOps", "NexCam", "NexPortal", "NexDocs", "NexReach", "NexComms", "NexSupply", "NexGuard", "NexFleet", "NexTime", "NexVision"];
...
function SubscriptionDetails({ entry, onBack }: { entry: SubscriptionPackage; onBack: () => void }): React.ReactElement {
  ...
  const [modules, setModules] = useState(nexTeamModules);
  const [message, setMessage] = useState("");
  return <section className="nexcommand__panel tenant-profile subscription-details"><button className="tenant-action" type="button" onClick={onBack}>Back to Subscription Roster</button><div className="tenant-profile__heading"><div className="tenant-profile__heading-copy"><p className="ui-eyebrow">Staging Subscription</p><div className="tenant-profile__tenant-line"><h2>{entry.name}</h2><span>$0</span></div><p>$0.00 monthly · $0.00 annually paid in full</p></div><button className="tenant-action" type="button" onClick={() => setEditing((value) => !value)}>{editing ? "Cancel" : "Edit Subscription"}</button></div><p className="subscription-details__notice">Testing preview only. Saving here does not create charges or change Stripe billing.</p><fieldset disabled={!editing}><div className="tenant-profile__grid"><label>Monthly Billing Price<span className="subscription-details__currency"><b>$</b><input inputMode="decimal" value={monthly} onChange={(event) => setMonthly(event.target.value)} /></span></label><label>Annual Billing Price (Paid in Full)<span className="subscription-details__currency"><b>$</b><input inputMode="decimal" value={annual} onChange={(event) => setAnnual(event.target.value)} /></span></label></div><div className="subscription-details__discount"><label className="subscription-details__toggle"><input type="checkbox" checked={promo} onChange={(event) => setPromo(event.target.checked)} /><span aria-hidden="true" /><div><strong>Promotion Discount</strong><small>{promo ? "Promotion is on for this staging preview." : "Promotion is off."}</small></div></label>{promo ? <div className="tenant-profile__grid"><label>Discount Type<select value={discountType} onChange={(event) => setDiscountType(event.target.value as typeof discountType)}><option value="percent">Percentage</option><option value="fixed">Fixed Dollar Amount</option></select></label><label>{discountType === "fixed" ? "Discount Amount" : "Discount Percentage"}<span className="subscription-details__currency"><b>{discountType === "fixed" ? "$" : "%"}</b><input inputMode="decimal" value={discountValue} onChange={(event) => setDiscountValue(event.target.value)} /></span></label></div> : null}</div><h3>Included NexTeam Modules</h3><div className="subscription-details__modules">{nexTeamModules.map((module) => <label key={module}><input type="checkbox" checked={modules.includes(module)} onChange={(event) => setModules((current) => event.target.checked ? [...current, module] : current.filter((value) => value !== module))} />{module}</label>)}</div></fieldset>{editing ? <button className="tenant-action" type="button" onClick={() => { setEditing(false); setMessage("Staging subscription preview saved for this browser. No billing records were changed."); }}>Save Staging Preview</button> : null}{message ? <p className="tenant-profile__message tenant-profile__message--success">{message}</p> : null}</section>;
}
