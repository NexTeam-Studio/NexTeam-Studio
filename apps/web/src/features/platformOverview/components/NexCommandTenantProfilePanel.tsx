...
export function NexCommandTenantProfilePanel({ user, tenantId, onBack }: { user: User | null; tenantId: string; onBack: () => void }): React.ReactElement {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      await request(user, `/api/platform/admin/tenants/${encodeURIComponent(tenantId)}/profile`, { method: "PATCH", body: JSON.stringify({ legalName: stringOrUndefined(form, "legalName"), dbaName: stringOrUndefined(form, "dbaName"), website: stringOrUndefined(form, "website"), status: form.get("status"), primaryContact: contactFrom(form), tenant: { name: form.get("name"), timezone: form.get("timezone"), lifecycleState: form.get("lifecycleState") } }) });
