...
type LifecycleRecord = {
  tenantOnboardingBlueprint: { id: string; prospectId: string; recommendedModules: string[]; createdAt: string };
...
async function load(user: User): Promise<{ tenantOnboardingBlueprints: LifecycleRecord[]; subscriptions: LifecycleRecord[]; onboarding: LifecycleRecord[] }> {
  const token = await user.getIdToken();
  const response = await fetch("/api/platform/admin/lifecycle", { headers: { authorization: `Bearer ${token}` } });
  const result = await response.json() as { error?: string; tenantOnboardingBlueprints?: LifecycleRecord[]; subscriptions?: LifecycleRecord[]; onboarding?: LifecycleRecord[] };
  if (!response.ok) throw new Error(result.error || "NexCommand could not load lifecycle records.");
  return { tenantOnboardingBlueprints: result.tenantOnboardingBlueprints ?? [], subscriptions: result.subscriptions ?? [], onboarding: result.onboarding ?? [] };
}

export function PlatformLifecycleRecordsPanel({ user, mode }: { user: User | null; mode: Mode }): React.ReactElement {
  const [data, setData] = useState<{ tenantOnboardingBlueprints: LifecycleRecord[]; subscriptions: LifecycleRecord[]; onboarding: LifecycleRecord[] }>({ tenantOnboardingBlueprints: [], subscriptions: [], onboarding: [] });
  ...
  const records = (mode === "blueprints" ? data.tenantOnboardingBlueprints : data[mode]).filter((record) => `${record.prospect?.businessName ?? ""} ${record.tenant?.name ?? ""} ${record.tenant?.id ?? ""}`.toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <section className="nexcommand__panel" aria-labelledby={`lifecycle-${mode}`}>
    ...
    <p>{mode === "blueprints" ? "Saved tenant onboarding plans and immutable revision history." : mode === "subscriptions" ? "Tenant package assignments, not the package catalog." : "Active tenant onboarding records. Secure setup remains unavailable until activation."}</p>
    ...
    <div className="nexcommand__directory">
      {records.map((record) => <article key={record.tenantOnboardingBlueprint.id}>
        <h3>{record.tenant?.name ?? record.prospect?.businessName ?? "Unlinked Prospect"}</h3>
        <dl className="nexcommand__facts">
          {mode === "blueprints" ? <><div><dt>Onboarding plan</dt><dd>{record.tenantOnboardingBlueprint.id}</dd></div><div><dt>Revisions</dt><dd>{record.revisions.length} · {record.revisions.map((revision) => `v${revision.revisionNumber} ${revision.approvalState}`).join(", ") || "None"}</dd></div><div><dt>Tenant</dt><dd>{record.tenant ? `${record.tenant.name} (${record.tenant.id})` : "Not Activated"}</dd></div></> : null}
          {mode === "subscriptions" ? <><div><dt>Tenant ID</dt><dd>{record.tenant?.id ?? "Not Activated"}</dd></div><div><dt>Package</dt><dd>Pilot Onboarding Package · $0.00</dd></div><div><dt>Status</dt><dd>{record.assignment?.status ?? "Not Assigned"}</dd></div><div><dt>Package Version</dt><dd>{record.assignment?.packageVersion ?? "Not Available"}</dd></div><div><dt>Enabled Modules</dt><dd>{record.tenantOnboardingBlueprint.recommendedModules.join(", ") || "Not Available"}</dd></div><div><dt>Activation</dt><dd>{record.tenant ? "Activated" : "Not Activated"}</dd></div></> : null}
