...
export function TenantOverviewPanel(props: { rows: PlatformTenantRow[]; onViewDetails: (tenantId: string) => void }): React.ReactElement {
  ...
  return <section className="tenant-overview" aria-label="Tenant roster">
    ...
    {visibleRows.map((row) => <article className="tenant-overview__row" key={row.tenant.id}>
      <div className="tenant-overview__identity"><span className="tenant-overview__logo" aria-hidden="true">{row.tenant.name.slice(0, 1).toUpperCase()}</span><div><h2>{row.tenant.name}</h2><p>{row.plan.name} · {row.subscription?.status ?? "no subscription"}</p><p><strong>{row.tenant.lifecycleState === "DISABLED_ARCHIVED" ? "Archived" : "Active"}</strong></p></div></div>
      ...
      <div className="tenant-overview__actions"><p>${row.cost.estimatedCostUsd.toFixed(2)} tracked</p><button type="button" onClick={() => props.onViewDetails(row.tenant.id)}>View details</button></div>
    </article>)}
