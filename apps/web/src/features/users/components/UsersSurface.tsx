...
function MemberEditor(props: { member: TeamMember; ownProfile?: boolean; canManageTeam: boolean; tenantId?: string; getAccessToken?: () => Promise<string>; onBack: () => void; onSave: () => void; saved: boolean }): React.ReactElement {
  ...
  return <main className="users-surface users-member-editor">
    ...
    <header className="users-editor-heading"><div>{props.ownProfile ? null : <p className="users-kicker">Team Member</p>}{props.ownProfile ? <h1 className="users-page-title"><PersonTitleIcon /> <span>My Profile</span></h1> : <h1>{props.member.name}</h1>}<div className="users-profile-identity"><Avatar member={props.member} large photoUrl={profileDraft.avatarDataUrl || undefined} /><label className="users-photo-action"><input type="file" onChange={(event) => { void selectProfilePhoto(event.target.files?.[0]); event.currentTarget.value = ""; }} />{profileDraft.avatarDataUrl ? "Change Photo from Files" : "Add Photo from Files"}</label><div><strong>{props.member.name}</strong><span>{props.member.title}</span></div></div></div></header>
