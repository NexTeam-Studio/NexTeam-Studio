...
- Root-level web helpers are no longer an informal ownership tier. Navigation/header/FAB/pattern-library code belongs to NexOps Shell; Home, Request Core, and the NexDocs client workspace have feature owners; branding, UI primitives, communication templates, intake presentation, and signature capture live in named shared components. Only the mixed deferred UI remains at web root pending its owner split.
...
- Field Docs and NexDocs Admin writes use the same fail-closed ownership rule. Create/upsert operations transactionally read the target document and reject an ID already owned by another tenant; mutable records require the caller tenant on update, validate stored ownership, and preserve the tenant identifier. Repository tests prove these boundaries against a deterministic Firestore transaction harness.
...
- Extend repository-bound enforcement to the remaining Firebase Admin repositories and add emulator-backed Admin SDK boundary tests. CRM/Tenant Config and Field Docs/NexDocs now enforce their highest-risk mutation boundaries, but Firestore rules do not constrain Admin SDK code and the Field Docs proof is not yet emulator-backed.
