...
- Admin-backed writes use a fail-closed ownership rule. Caller-selected IDs transactionally read the target document and reject an ID already owned by another tenant; mutable records require caller tenant context, validate stored ownership, and preserve the tenant identifier. `check:admin-tenant-writes` blocks direct or unclassified Admin SDK writes, and the opt-in Firestore emulator suite proves the shared seam plus ApprovalQueue, CRM, Platform, and Field Docs against the real transaction engine.
...
- `check-tenancy` remains a static read guard, while `check-admin-tenant-writes` audits server-side Admin mutations. Runtime proof comes from deterministic repository tests and `admin-tenant-isolation.emulator.test.mjs`; the emulator suite is opt-in and never runs as part of the default no-browser test command.
