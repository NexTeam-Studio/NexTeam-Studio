...
| core | M0.7 verify + receipts | blocked | Local verify/smoke/history scan done in `receipts/m0/verify.txt`, `receipts/m0/smoke-m0.txt`, and `receipts/security/history-scan.txt`; live `/api/version` SHA-match blocked until push + separate Railway environment |
