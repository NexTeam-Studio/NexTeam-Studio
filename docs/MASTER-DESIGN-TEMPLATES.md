...
| `NexOpsRosterTemplate` | The archive/list Page Template for NexOps module roster Pages. Provides the approved roster Hero, Page action region, search/filter slot, result banner, expandable roster-record styling, and roster content region. | `apps/web/src/shared/ui/NexOpsBusinessTemplates.tsx` and `NexOpsBusinessTemplates.css` | Shared across current NexOps roster Pages; Clients, Quotes, Jobs, Requests, and Invoices are current consumers. |
