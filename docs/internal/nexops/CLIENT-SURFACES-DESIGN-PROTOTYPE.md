...
## NexSuite Global layout-parts contract

- **Pass-down authority:** Approved behavior under `Templates → Design → NexSuite → Global` is the authoritative shared pattern for applicable NexTeam application surfaces. Prefer one shared Global component or primitive over per-worktree visual recreation.
- **Directly dependent adjacent work:** A Global layout-part implementation may change an adjacent NexCommand or NexSuite surface only when that change is materially necessary to design, implement, validate, or preserve the in-scope Header, Sidebar, Application Shell, Footer, or later approved Global layout part. Examples include route and hierarchy placement, shared shell integration, responsive coordination, active navigation, layout offsets, and shared tokens.
- **Boundary:** This is not blanket cross-worktree authority. It does not authorize unrelated feature work, business logic, opportunistic cleanup, broad refactors, or an adjacent change with no demonstrable Global-layout dependency.
- **Hierarchy:** Global layout work may adjust NexSuite/NexCommand navigation grouping, nesting, ordering, and parent-child relationships where that hierarchy is necessary to the approved Global design.
- **Review evidence:** Every use of this authority must name the adjacent files and surfaces, state the direct dependency, confirm unrelated behavior was left unchanged, include affected visible surfaces in browser validation, and include the full affected set in the exact-SHA review scope. If there are none, state `Adjacent/side-loaded changes: None.`
