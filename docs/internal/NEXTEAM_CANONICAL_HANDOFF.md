...
| Provisioner/auth/tenant-isolation proof level | `BUILT+PROVEN` | Corrected and restored on `2026-07-14` after the NexCam rename verification pass exposed stale proof drift. `agentSessions` is a live intentional lane (`/agent-architect` public surface plus `/admin/sessions` operator view), so the fix restored Firestore coverage instead of deleting it. Current proof now includes green static rules coverage `4/4` in `src/features/tenancy/services/firestoreRulesTenantIsolation.test.mjs` plus behavior-based emulator coverage `21/21` in `src/features/tenancy/services/firestoreTenantRuntimeEmulator.test.mjs`, including explicit proof that public-session claims are blocked from `tenants/{tenantId}/...` foundation documents while session owners can access only their own `agentSessions` docs and platform operators can oversee them. | `src/App.jsx`, `src/features/agentArchitect/services/firestoreSession.js`, `src/features/admin/services/adminfirestore.js`, `src/features/auth/services/firebaseTenantAuthService.js`, `firestore.rules`, `src/features/tenancy/services/firestoreRulesTenantIsolation.test.mjs`, `src/features/tenancy/services/firestoreTenantRuntimeEmulator.test.mjs` |
