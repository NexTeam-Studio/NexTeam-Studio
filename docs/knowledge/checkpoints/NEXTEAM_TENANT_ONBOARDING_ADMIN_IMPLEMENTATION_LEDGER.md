...
| Today Goal 2 — persistent tenant subscription lifecycle | GREEN | target-architecture-integration | Reuses the authoritative platform tenant/owner/subscription/audit path. Two owner-confirmed server validations archive without deletion; production Firebase access checks and Firestore rules deny archived tenant access; resubscribe appends a new immutable subscription for the same tenant and owner linkage. Focused platform tests, full non-browser tests (515 pass, 0 fail, 3 skipped), typecheck, lint, build, tenancy, indexes, worktree, and secret gates passed on 2026-08-10. Contract: `docs/contracts/tenant-subscription-lifecycle.md`. |
| Today Goal 1 — Gmail-independent tenant owner activation | GREEN (identity preflight blocked) | target-architecture-integration | The existing platform activation transaction now atomically commits the tenant, passwordless/reused Firebase owner linkage, tenant-user profile, active subscription, prospect/assignment transition, and membership audit. Non-tenant Firebase claims are retained; NexCommand’s existing lifecycle projection exposes authorized owner/onboarding status without provider failure detail; production tenant routes require an explicit tenant assignment rather than a platform-owner fallback. The non-secret staging owner-invitation Gmail identity is platform-scoped and visible only through sanitized Credentials & Provider Management status. Its OAuth client/project mapping is unproven, so `SAFE_TO_REAUTHORIZE=false`; sending and delivery remain quarantined. Contracts: `docs/contracts/tenant-owner-activation.md`, `docs/contracts/staging-owner-invitation-gmail-provider.md`; receipt: `receipts/architecture/NEXTEAM-TODAY-GOAL1-STAGING-GMAIL-PREFLIGHT-20260810.md`. |
