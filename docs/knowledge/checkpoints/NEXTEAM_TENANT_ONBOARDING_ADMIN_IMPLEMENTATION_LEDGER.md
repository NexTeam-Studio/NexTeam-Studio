...
| Today Goal 1 — Gmail-independent tenant owner activation | GREEN | target-architecture-integration | The existing platform activation transaction now atomically commits the tenant, passwordless/reused Firebase owner linkage, tenant-user profile, active subscription, prospect/assignment transition, and membership audit. Non-tenant Firebase claims are retained; NexCommand’s existing lifecycle projection exposes authorized owner/onboarding status without provider failure detail; production tenant routes require an explicit tenant assignment rather than a platform-owner fallback. Gmail configuration, sending, and delivery verification remain quarantined. Contract: `docs/contracts/tenant-owner-activation.md`; receipt: `receipts/architecture/NEXTEAM-TODAY-GOAL1-GMAIL-INDEPENDENT-20260810.md`. |
